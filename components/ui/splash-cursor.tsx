  ...
  useEffect(() => {
    ...
    const advectionShader = compileShader(
      ...
      void main () {
              ...
              vec2 coord = vUv - uScrollOffset - dt * bilerp(uVelocity, vUv, texelSize).xy * texelSize;
              vec4 result = bilerp(uSource, coord, dyeTexelSize);
          #else
              vec2 coord = vUv - uScrollOffset - dt * texture2D(uVelocity, vUv).xy * texelSize;
    ...
    function step(dt: number) {
      ...
      if (advectionProgram.uniforms.uScrollOffset) {
        const pixelRatio = window.devicePixelRatio || 1;
        const offsetY = (scrollDeltaY * pixelRatio) / canvas!.height;
        gl.uniform2f(advectionProgram.uniforms.uScrollOffset, 0.0, offsetY);
      }
    ...
    function onScroll() {
       ...
       const pointer = pointers[0];
       if (pointer && (pointer.texcoordX !== 0 || pointer.texcoordY !== 0)) {
           const deltaYTex = -dy / window.innerHeight; 
           const deltaY = correctDeltaY(deltaYTex);
           splat(
               pointer.texcoordX, 
               pointer.texcoordY, 
               0, 
    ...
    return () => {
        ...
        // Cleanup loop! I will add this to prevent memory leak
